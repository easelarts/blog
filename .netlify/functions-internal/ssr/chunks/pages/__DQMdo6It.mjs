import { $ as $$BaseHead, a as $$Header, b as $$Footer } from './404_Y-y4M5Iz.mjs';
...
const $$BlogItem = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<li${addAttribute(`${index === 0 ? "w-full" : "w-full md:w-1/2"} pl-2 pt-2 text-center decoration-0 ease-linear duration-200 first:mb-0 md:first:mb-4 first:text-center  md:first:text-2xl `, "class")}> <a class="m-0"${addAttribute(`/${post.slug}/`, "href")}> ${post.image && renderTemplate`<img class=" w-auto mb-2 rounded-lg"${addAttribute(post.image.filename, "src")} alt="">`} <h4 class="m-0 text-light-text dark:text-dark-text leading-none">${post.title}</h4> ${renderComponent($$result, "FormattedDate", $$FormattedDate, { "date": new Date(post.date) })} </a> </li>`;
...
const $$BlogPostList = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<main class="flex flex-col items-center min-h-fit w-full"> <section class="lg:w-5/12 w-4/5 ease-linear duration-200"> <ul class="flex flex-wrap list-none m-0 p-0 w-full"${spreadAttributes(ke(blok))}> ${posts.map((post, index) => renderTemplate`${renderComponent($$result, "BlogItem", $$BlogItem, { "post": post, "index": index })}`)} </ul> </section> </main>`;
...
const $$ = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const story = data.story;
  return renderTemplate`<html lang="en"> <head><title>Easel Arts</title>${renderHead()}</head> <body> ${renderComponent($$result, "StoryblokComponent", $$StoryblokComponent, { "blok": story.content, "updatedDate": story.published_at })}</body></html>`;
